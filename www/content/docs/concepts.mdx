...
**Better Convex solves this.** Convex's reactive database + TanStack Query's DX:

- **tRPC-style API** — Fluent builder with full type inference
- **TanStack Query Native** — `useQuery`, `useMutation`, DevTools
...
Convex already has real-time subscriptions. Better Convex bridges them into TanStack Query so you get WebSocket updates **and** the full TanStack Query API — `isPending`, `isError`, DevTools, cache invalidation, optimistic updates — without manual wiring.

With vanilla Convex, you'd use `@convex-dev/react-query` and wire each query with `convexQuery(api.path, args)`. Better Convex includes the adapter out of the box and generates typed query options from your procedures:
...
_Define once, use everywhere. Need admin-only? Extend `authQuery` with a role check._
...
import { defineMigration, defineMigrationSet } from 'better-convex/orm';
...
| `convex/functions/generated/*` | Better Convex callers and runtime contracts |
| `convex/shared/api.ts` | Typed API metadata for the client |

Dev loop: change schema/procedures → `better-convex dev` → use generated artifacts.
