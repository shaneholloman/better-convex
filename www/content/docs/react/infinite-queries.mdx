...
**Important:** Always call `.paginated({ limit, item })` before `.query()`. The handler receives flat `input.cursor` and `input.limit` which you pass through to ORM `paginate: { cursor, numItems }`.
</Callout>
