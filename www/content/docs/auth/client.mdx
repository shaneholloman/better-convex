...
If you're using `@convex-dev/auth` (common in React Native) instead of better-auth, wrap your app with `ConvexAuthBridge`:

```tsx title="App.tsx" showLineNumbers {1,6-8}
import { ConvexAuthBridge } from 'better-convex/react';
...
<Callout icon={<InfoIcon />}>
`ConvexAuthBridge` must be placed inside `ConvexProviderWithAuth`.
</Callout>
