...
Function examples use Better-Convex ORM via `ctx.orm`.
</Callout>
    ...
    const orgs = (
      await asyncMap(members, async (member) => {
        const org = await ctx.orm.query.organization.findFirst({
          where: { _id: member.organizationId },
        });
        if (!org) return null;
        return {
          id: org._id,
...
export const update = authMutation
  ...
  .mutation(async ({ ctx, input }) => {
    ...
    await ctx.orm
      .update(organization)
      .set(input)
      .where(eq(organization._id, ctx.user.activeOrganization!.id));
...
export const listUserInvitations = authQuery
  ...
  .query(async ({ ctx }) => {
    ...
    return asyncMap(invitations, async (inv) => {
      const org = await ctx.orm.query.organization.findFirst({
        where: { _id: inv.organizationId },
      });
      const inviter = await ctx.orm.query.user.findFirst({
        where: { _id: inv.inviterId },
      });
      return {
        id: inv._id,
...
export const listPendingInvitations = authQuery
  ...
  .query(async ({ ctx, input }) => {
    ...
    const invitations = await ctx.orm.query.invitation.findMany({
      where: { organizationId: org._id, status: 'pending' },
      limit: 100,
    });
...
export const updateMemberRole = authMutation
  ...
  .mutation(async ({ ctx, input }) => {
    ...
    await ctx.orm
      .update(member)
      .set({ role: input.role })
      .where(eq(member._id, input.memberId));
