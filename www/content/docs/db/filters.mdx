...
export const search = publicQuery
  ...
  .query(async ({ ctx, input }) => {
    return await ctx.table.query.articles.findMany({
      search: {
...
export const searchCharacters = publicQuery
  ...
  .query(async ({ ctx, input }) => {
    return await ctx.table.query.characters.findMany({
      where: (char) => {
...
Predicate `where` runs post‑fetch against an explicit index stream. Use `maximumRowsRead` to cap scan size:

```ts showLineNumbers {2-6}
const results = await db.query.posts.findMany({
  where: (post) => expensiveCheck(post),
  index: { name: 'by_created_at' },
  paginate: { cursor: input.cursor, numItems: input.limit, maximumRowsRead: 500 },
});
```
...
- Use selective index ranges for predicate `where` and cap reads with `maximumRowsRead`
...
4. **Bound predicate scans** - Use explicit `index` and cap reads with `maximumRowsRead`
