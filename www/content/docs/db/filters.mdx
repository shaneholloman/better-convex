...
- **Full-text search**: Search string fields with relevance scoring and fast filtering
- **Streams**: Complex filtering with consistent pagination from [convex-helpers](https://github.com/get-convex/convex-helpers)
- **SQL-like patterns**: UNION, JOINs, and DISTINCT operations

<Callout type="warning">
Streams do NOT support `withSearchIndex()`. You cannot combine streams with full-text search - choose one approach based on your needs.
</Callout>
...
Add search indexes to your schema using the naming pattern `search_<searchField>`:
...
<Callout type="info">
Each search index can only search ONE field. Add `filterFields` for fast equality filtering before text search.
</Callout>

### Basic Search

Search a field and limit results:

```ts title="convex/functions/articles.ts"
import { z } from 'zod';
...
<Callout>
Always use `.take()` or `.paginate()` with search queries to limit results.
</Callout>
...
<Callout type="info">
Search results include a `_score` field for relevance ranking. Results are automatically sorted by relevance.
</Callout>
...
<Callout type="warning">
Stream initialization requires `ctx.db` as the first parameter (exception to the [ctx.table rule](/docs/db/ents)). However, you SHOULD use `ctx.table()` inside stream callbacks for lookups.
</Callout>

### filterWith - Consistent Page Sizes

The built-in `.filter()` causes variable page sizes with pagination because it filters AFTER fetching. Use `filterWith` to filter BEFORE pagination for consistent pages:

```ts title="convex/functions/characters.ts"
...
<Callout>
`filterWith` supports any TypeScript logic including async lookups, array methods, and complex conditions that aren't possible with Convex's built-in filter expressions.
</Callout>
...
```ts title="convex/functions/messages.ts"
import { stream, mergedStream } from 'convex-helpers/server/stream';
...
<Callout type="info">
All streams being merged MUST be ordered by the same fields specified in the merge. The second parameter `['_creationTime']` tells the merger how to interleave results.
</Callout>
...
<Callout type="warning">
Without `maximumRowsRead`, a filter that matches few documents could scan the entire table. Set a reasonable limit to fail fast instead of timing out.
</Callout>
