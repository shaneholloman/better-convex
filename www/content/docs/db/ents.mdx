...
| Issue | Solution |
|-------|----------|
| `ctx.db` still used | Use `ctx.db` for search indexes or raw Convex queries |
...
<Callout icon={<InfoIcon />}>
**Tip:** Use `ctx.table` by default. Use `ctx.db` for search indexes or raw Convex queries.
</Callout>
