...
ORM `findMany()` must be explicitly sized: use `limit`, `paginate`, set a schema `defaultLimit`, or opt in with `allowFullScan: true`.
</Callout>
...
<Compare>
  ...
  <CompareItem title="After (ORM)">
    ...
    ```ts showLineNumbers
    // Patch-style update:
    await ctx.orm.update(users).set({ name: input.name }).where(eq(users._id, input.userId));
