...
|---|---|---|---|---|
| `findMany({ where: object })` | Optional if fully index-compiled; otherwise call `.withIndex(...)` | `limit/offset`, `cursor + limit` | Yes (`where: { relation: ... }`) | `orderBy` supported; first field drives cursor stability |
| `findMany({ where: (table, ops) => ... })` | Optional if fully index-compiled; otherwise call `.withIndex(...)` | `limit/offset`, `cursor + limit` | No (base-table expression only) | `orderBy` supported; first field drives cursor stability |
...
2. **Need complex filters with pagination?** → Use predicate `where` + explicit `.withIndex(...)`
3. **Need range queries (greater than, less than)?** → Use predicate `where` + explicit `.withIndex(...)`
