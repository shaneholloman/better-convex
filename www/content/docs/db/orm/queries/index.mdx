...
Compound indexes follow Convex prefix rules. For an index on `[type, numLikes]`, `where: { numLikes: 10 }` is treated as non-indexable and requires explicit `.withIndex(...)` (and `maxScan` in cursor mode) unless you also constrain the leading field (`type`). Reversed `AND` equality order is normalized to index field order before query execution.
...
These operators are non-index-compiled and require explicit `.withIndex(...)` unless paired with another indexable pre-filter. Here are scalable workarounds for each:
