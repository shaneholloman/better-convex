...
**Note**: Non-paginated `findMany()` requires explicit sizing: provide `limit`, use `paginate`, set `allowFullScan: true`, or configure `defineSchema(..., { defaults: { defaultLimit } })`.
</Callout>
...
Compound indexes follow Convex prefix rules. For an index on `[type, numLikes]`, `where: { numLikes: 10 }` is treated as non-indexable and requires `allowFullScan: true` unless you also constrain the leading field (`type`). Reversed `AND` equality order is normalized to index field order before query execution.
...
These operators are non-index-compiled and require `allowFullScan: true` unless paired with another indexable pre-filter. Here are scalable workarounds for each:
...
<Callout icon={<InfoIcon />}>
**Note**: For `many()` relations, provide `with.<relation>.limit` or set `defineSchema(..., { defaults: { defaultLimit } })`. Otherwise, use `allowFullScan: true` on the parent query.
</Callout>
