...
### Migrate component data to app namespace

<Callout icon={<AlertTriangle />}>
**Warning:** This ID remap flow is validated for the latest Convex document ID format (`id_v6`, footer version `0`). Older historical ID formats may exist and are not guaranteed by this script.
</Callout>
...
**2) Install migration zip dependencies**

```bash
npm i -D yazl yauzl
```

**3) Run the migration utility (creates a new `migration.zip`)**

```bash
node --experimental-strip-types ./tooling/migrations/better-auth-component/cli.ts \
